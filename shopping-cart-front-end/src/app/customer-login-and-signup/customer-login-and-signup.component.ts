...
@Component({
  selector: 'app-customer-login-and-signup',
  templateUrl: './customer-login-and-signup.component.html',
  styleUrls: ['./customer-login-and-signup.component.scss']
})
export class CustomerLoginAndSignupComponent implements OnInit {
  ...
  onSubmit(){
      ...
      this.service.registerUser(user).subscribe(data=>{
        ...
        if(this.newUser == 1){
          console.log("success !");
          this._router.navigate(['/customer/loginandsignup/auth']);
          console.log("code ..");
        }
        else{
